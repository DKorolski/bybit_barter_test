...
const FILE_PATH_SYSTEM_CONFIG: &str = "barter/examples/config/system_config_bybit_perpetual.json";
const RISK_FREE_RETURN: Decimal = dec!(0.05);

/// Example showing how to feed Bybit OrderBook L2 depth + public trades into the synchronous
/// Engine. This variant only consumes live market data (no execution link configured) so you can
/// observe how the engine ingests OrderBook and trade updates before adding your own execution
/// client or strategy logic.
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Initialise MarketData Stream with Bybit OrderBook L2 + public trades
    ...
    let mut system = SystemBuilder::new(args)
        ...
        // Start with trading enabled to immediately consume live data
    ...
    // Run AuditStream consumer that logs snapshots
    let audit_task = tokio::spawn(async move {
        let mut audit_stream = audit.updates.into_stream();
        while let Some(audit) = audit_stream.next().await {
            info!(?audit, "Audit snapshot");
    ...
    // Let the example run for 10 seconds...
